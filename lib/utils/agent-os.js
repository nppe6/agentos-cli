...
function collectProjectionTemplates(targetDirectory, tools = DEFAULT_TOOLS, stack = 'core') {
  ...
  const codexPromptsDirectory = path.join(agentOsDirectory, 'templates', 'codex-prompts');
  ...
  for (const tool of selectedTools) {
    ...
    if (layout.capabilities && layout.capabilities.promptCommands && fs.existsSync(codexPromptsDirectory)) {
      for (const promptFile of collectFiles(codexPromptsDirectory, codexPromptsDirectory)) {
        templates.push({
          path: normalizeRelativePath(path.join(layout.rootDirectory, 'prompts', promptFile)),
          sourcePath: path.join(codexPromptsDirectory, promptFile),
          tool
        });
      }
    }
...
function getAgentTransform(tool, agentFile) {
  ...
  if (normalizedAgentFile === 'shelf-implement.md') {
    return (content) => injectCodexAgentPrelude(content, 'implement');
  }
  if (normalizedAgentFile === 'shelf-check.md') {
