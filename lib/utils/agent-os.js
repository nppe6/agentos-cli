...
function collectProjectionTemplates(targetDirectory, tools = DEFAULT_TOOLS, stack = 'core') {
  ...
  const claudeTemplatePath = resolveTemplateSourceFile(targetDirectory, stack, path.join('templates', 'CLAUDE.md'));
  const claudeSettingsPath = resolveTemplateSourceFile(targetDirectory, stack, path.join('templates', 'claude-settings.json'));
  const bundledSkillsDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, path.join('templates', 'bundled-skills'));
  const commonCommandsDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, path.join('templates', 'common-commands'));
  const commonSkillsDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, path.join('templates', 'common-skills'));
  ...
  const codexHooksDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, path.join('templates', 'codex-hooks'));
  const sharedHooksDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, path.join('templates', 'shared-hooks'));
  const customSkillsDirectory = path.join(agentOsDirectory, 'skills');
  const agentsDirectory = resolveTemplateSourceDirectory(targetDirectory, stack, 'agents');
  ...
  for (const tool of selectedTools) {
    ...
    if (tool === TOOL_CODEX && layout.skillsDirectory) {
      if (commonCommandsDirectory) {
        for (const commandFile of collectFiles(commonCommandsDirectory, commonCommandsDirectory)) {
          if (!shouldProjectCodexCompatibilityCommandSkill(commandFile)) {
            continue;
          }

          const skillName = path.basename(commandFile, path.extname(commandFile));
          templates.push({
            path: normalizeRelativePath(path.join(layout.skillsDirectory, skillName, 'SKILL.md')),
            sourcePath: path.join(commonCommandsDirectory, commandFile),
            transform: getCodexCompatibilityCommandSkillTransform(commandFile),
            tool
          });
        }
      }
      ...
      for (const skillFile of commonSkillTemplates) {
        ...
        templates.push({
          path: normalizeRelativePath(path.join(layout.skillsDirectory, skillName, 'SKILL.md')),
          sourcePath: path.join(commonSkillsDirectory, skillFile),
          transform: getCodexCompatibilityCommonSkillTransform(skillFile),
          tool
        });
      }
    }
...
function shouldProjectCodexCompatibilityCommandSkill(commandFile) {
  const baseName = path.basename(commandFile).toLowerCase();
  return baseName === 'start.md' || baseName === 'finish-work.md';
}
...
function supportsSharedToolScopedSkills(tool, layout) {
  return Boolean(layout.capabilities && layout.capabilities.toolScopedSkills && tool !== TOOL_CODEX);
}
...
function getCodexCompatibilityCommandSkillTransform(commandFile) {
  const commandName = path.basename(commandFile, path.extname(commandFile));

  return (content) => wrapCommandAsSkill(
    commandName,
    getCommandSkillDescription(commandName),
    resolveTemplatePlaceholders(content, TOOL_CODEX)
  );
}
...
function getCodexCompatibilityCommonSkillTransform(skillFile) {
  const skillName = path.basename(skillFile, path.extname(skillFile));

  return (content) => wrapCommonSkill(
    skillName,
    getCommonSkillDescription(skillName),
    resolveTemplatePlaceholders(content, TOOL_CODEX)
  );
}
