...
function resolveSyncScriptSource(stack) {
  const coreScript = path.join(CORE_TEMPLATE_ROOT, 'scripts', 'sync-agent-os.ps1');
  if (fs.existsSync(coreScript)) {
    return coreScript;
  }

  return path.join(resolveStackRoot(stack), 'scripts', 'sync-agent-os.ps1');
}
