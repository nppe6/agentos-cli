...
function getSharedHookScriptsForPlatform(platform, options) {
  ...
  const sharedHookFilesByName = typeof options === 'string'
    ? null
    : (options.sharedHookFilesByName || null);
  ...
  return hookNames.map((hookName) => ({
    name: hookName,
    sourcePath: sharedHookFilesByName && sharedHookFilesByName[hookName]
      ? sharedHookFilesByName[hookName]
      : path.join(sharedHooksDirectory, hookName)
  }));
}
