...
async function agentInit(target = '.', options = {}, dependencies = {}) {
  ...
  syncAgentOs(targetDirectory, selectedTools, stack);
  ...
  if (shouldUseAgentOs) {
    console.log(colorize('cyan', '提示：当前选择了多个 Agent 工具，已启用 .agent-os 统一源并直接生成各工具投影。'));
  }
  else {
    console.log(colorize('green', '提示：当前只选择了一个 Agent 工具，仅生成对应工具配置。'));
  }
