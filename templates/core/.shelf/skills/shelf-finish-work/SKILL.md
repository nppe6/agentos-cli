...
```bash
python3 ./.shelf/scripts/get_context.py --mode record
```
...
Filter out paths under `.shelf/workspace/` and `.shelf/tasks/` — those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this skill's own work.
...
```bash
python3 ./.shelf/scripts/task.py archive <task-name>
```
