...
1. `.shelf/workflow.md`
2. `.shelf/scripts/get_context.py`
...
| Source | Purpose |
| --- | --- |
| `.shelf/workflow.md` | Workflow and next-action hints. |
| `.shelf/tasks/<task>/prd.md` | Current task requirements. |
| `.shelf/tasks/<task>/implement.jsonl` | Spec/research to read before implementation. |
| `.shelf/tasks/<task>/check.jsonl` | Spec/research to read during checking. |
| `.shelf/spec/` | Project specs. |
| `.shelf/workspace/` | Session records. |
...
```jsonl
{"file": ".shelf/spec/backend/index.md", "reason": "Backend conventions"}
{"file": ".shelf/tasks/04-28-x/research/api.md", "reason": "API research"}
```
...
- `.shelf/scripts/common/session_context.py`
...
- agent pull: edit the read steps in the corresponding `agentos-implement` / `shelf-check` agent file.
...
python3 ./.shelf/scripts/task.py list-context <task>
