...
When the user wants to change the engineering conventions AI follows, add new spec layers, or adjust monorepo package mapping, edit `.shelf/spec/` and `.shelf/config.yaml`.

## Read These Files First

1. `.shelf/config.yaml`
2. `.shelf/spec/`
3. `.shelf/workflow.md` Phase 1.3 and Phase 3.3
...
| Need | Edit location |
| --- | --- |
| Add backend/frontend/docs/test spec layer | `.shelf/spec/<layer>/` or `.shelf/spec/<package>/<layer>/` |
| Add shared thinking guides | `.shelf/spec/guides/` |
| Adjust monorepo packages | `packages` in `.shelf/config.yaml` |
| Change default package | `default_package` in `.shelf/config.yaml` |
| Control spec scanning scope | `spec_scope` in `.shelf/config.yaml` |
...
```bash
python3 ./.shelf/scripts/task.py add-context <task> implement ".shelf/spec/webapp/security/index.md" "Security conventions"
python3 ./.shelf/scripts/task.py add-context <task> check ".shelf/spec/webapp/security/index.md" "Security review rules"
```

## Change Monorepo Packages

Example `.shelf/config.yaml`:
