...
If the user wants only one platform to avoid sub-agents, first confirm whether that platform has a separate group in the workflow. Then change Phase 2 routing for that platform group instead of deleting all `agentos-implement` / `shelf-check` instructions across platforms.

## `/agentos:continue` Route Table

`/agentos:continue` resumes a task by deciding which phase step to load next. The decision combines `task.json.status` with the presence of artifacts inside the task directory. The mapping is fixed in the command itself; forks that add custom statuses must extend both the workflow.md tag block and this table.

| `status` | Artifact state | Resume at |
| --- | --- | --- |
| `planning` | `prd.md` missing | Phase 1.1 (load `shelf-brainstorm`) |
...
| `in_progress` | no implementation in conversation history | Phase 2.1 (`agentos-implement`) |
| `in_progress` | implementation done, no `shelf-check` run | Phase 2.2 (`shelf-check`) |
| `in_progress` | check passed | Phase 3.1 (verify quality + spec update) |
| `completed` | task is still in active tree | Phase 3.5 (run `/agentos:finish-work` to archive) |

When you add a custom status (e.g. `in-review`), add a `[workflow-state:in-review]` block in `.shelf/workflow.md` for the per-turn breadcrumb AND extend this route table — usually by editing the `/agentos:continue` command file (`.{platform}/commands/agentos/continue.md` or equivalent) to add a row that decides where to resume from. Without the route entry, `/agentos:continue` will fall through to a default branch and the user will not land on the step you intended.

## Notes

`.shelf/workflow.md` is the local project workflow, not an immutable template. The user can adapt it to team habits. After editing it, platform entry files may still contain old descriptions, so inspect them too.
