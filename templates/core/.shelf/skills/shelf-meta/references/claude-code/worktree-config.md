...
Complete guide to `.shelf/worktree.yaml` configuration.
...
**Note**: AgentOS Shelf uses a custom YAML parser (not PyYAML). Supports basic key-value pairs and arrays; complex nested structures may not work.
...
AgentOS Shelf uses a custom YAML parser (not PyYAML) with these limitations:
