...
```bash
TASK_DIR=$(python ./.shelf/scripts/task.py create "brainstorm: <short goal>" --slug <auto>)
```
...
* Check configs, existing command definitions, and project documentation
...
Delegate to `shelf-research` when research depth is needed. Keep the main conversation focused on synthesizing the answer, not dumping raw research logs inline.
...
After brainstorm completes (Step 8 confirmation approved), the flow continues to the task workflow's implementation preparation and execution phases.
