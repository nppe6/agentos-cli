...
```bash
{{PYTHON_CMD}} ./.shelf/scripts/get_context.py --mode record
```
...
Filter out paths under `.shelf/workspace/` and `.shelf/tasks/` -- those are managed by `add_session.py` and `task.py archive` auto-commits and will appear dirty as part of this command's own work.
...
```bash
{{PYTHON_CMD}} ./.shelf/scripts/task.py archive <task-name>
```
...
```bash
{{PYTHON_CMD}} ./.shelf/scripts/add_session.py \
  --title "Session Title" \
  --commit "hash1,hash2" \
