...
The AgentOS Shelf task system is stored entirely under `.shelf/tasks/` in the user project. Each task is a directory containing requirements, context, research, state, and relationship information.
...
The user sees a "current task," but AgentOS Shelf stores active task state per session.

```text
.shelf/.runtime/sessions/<context-key>.json
```
...
```jsonl
{"file": ".shelf/spec/cli/backend/index.md", "reason": "Backend conventions"}
{"file": ".shelf/tasks/04-28-example/research/api.md", "reason": "API research"}
```
...
```bash
python3 ./.shelf/scripts/task.py create "<title>" --slug <slug>
python3 ./.shelf/scripts/task.py start <task>
python3 ./.shelf/scripts/task.py current --source
python3 ./.shelf/scripts/task.py add-context <task> implement <file> <reason>
python3 ./.shelf/scripts/task.py validate <task>
python3 ./.shelf/scripts/task.py finish
python3 ./.shelf/scripts/task.py archive <task>
```
...
| Need | Edit location |
| --- | --- |
| Change the default task template | `.shelf/scripts/common/task_store.py` and task creation instructions. |
| Change status semantics | `.shelf/workflow.md`, workflow-state hook logic, and task usage conventions. |
| Add task lifecycle actions | `hooks.after_*` in `.shelf/config.yaml`. |
| Change context rules | Phase 1.3 in `.shelf/workflow.md` and related platform agent/hook instructions. |
| Change archive policy | `.shelf/scripts/common/task_store.py` / `task_utils.py`. |

These are local files in the user project. Do not default to editing AgentOS Shelf CLI source code unless the user wants to contribute upstream.
