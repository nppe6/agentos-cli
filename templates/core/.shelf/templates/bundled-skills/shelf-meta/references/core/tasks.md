...
```jsonl
{"file": ".shelf/spec/cli/backend/index.md", "reason": "Backend guidelines"}
{"file": "src/services/auth.ts", "reason": "Existing auth service"}
{"file": ".shelf/tasks/01-31-add-login/prd.md", "reason": "Requirements"}
```
...
### `.shelf/.runtime/sessions/<session-key>.json`

Stores the active task for one AI session/window.

```json
{
  "current_task": ".shelf/tasks/01-31-add-login"
}
```
...
python3 .shelf/scripts/task.py create "Child task" --slug child --parent <parent-dir>
```
...
python3 .shelf/scripts/task.py list --mine
python3 .shelf/scripts/task.py list --status planning
```
...
```bash
python3 .shelf/scripts/task.py add-subtask <parent-dir> <child-dir>
```
...
```bash
python3 .shelf/scripts/task.py remove-subtask <parent-dir> <child-dir>
```
...
```bash
python3 .shelf/scripts/task.py set-branch <dir> <branch>
python3 .shelf/scripts/task.py set-base-branch <dir> <branch>
python3 .shelf/scripts/task.py set-scope <dir> <scope>
python3 .shelf/scripts/task.py add-context <dir> <jsonl> <path> [reason]
python3 .shelf/scripts/task.py validate <dir>
python3 .shelf/scripts/task.py list-context <dir>
python3 .shelf/scripts/task.py list-archive [month]
python3 .shelf/scripts/task.py create-pr [dir] [--dry-run]
```
...
```bash
python3 .shelf/scripts/get_context.py                      # Default text (full context)
python3 .shelf/scripts/get_context.py --json                # Default JSON
python3 .shelf/scripts/get_context.py --mode record         # Record text (my tasks focus)
python3 .shelf/scripts/get_context.py --mode record --json  # Record JSON
```
